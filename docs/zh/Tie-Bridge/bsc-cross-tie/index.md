...
本指南介绍BSC TEST（币安测试链）和 TIE链之间的跨链交易实施
...
我们将通过命令将合约部署到 BSC TEST 链。该命令部署所有合约，包括 Bridge、ERC20 Handler、ERC721 Handler、Generic Handler、ERC20 和 ERC721 合约。此外，它还会设置默认的中继帐户地址和阈值。
